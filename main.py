...
def set_app_icon(base_dir):
    icon_path = os.path.join(base_dir, "App", "cutongbg.ico")
    if os.path.exists(icon_path) and os.path.isfile(icon_path):
        return icon_path
    else:
        print("Warning: Application icon 'cutongbg.ico' not found.")
        return None
...
def main():
    ...
    # Handle --check-driver flag
    if '--check-driver' in sys.argv:
        print(f"ChromeDriver path: {driver_path}")
        sys.exit(0 if os.path.exists(driver_path) else 1)
    ...
    try:
        from App.cutongbg import run_app
        print("Starting CutongBG application...")
        run_app(BASE_DIR, icon_path)
    except ImportError as e:
        print(f"Error importing CutongBG application: {e}")
        return 1
